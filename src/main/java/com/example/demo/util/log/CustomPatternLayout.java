...
public class CustomPatternLayout extends AbstractStringLayout{
    ...
    public static String customize(String text) {
        return LogTuoMinUtil.getTuoMinMsg(text);
    }
     ...
     *                              does not specify so).
     * @param noConsoleNoAnsi       If {@code "true"} (default) and
     ...
     * <li>Key: "formatType" Value: "conversion" (format uses the keywords supported
     * by OptionConverter)</li>
    ...
    /**
     * Creates a text representation of the specified log event and writes it into
     * the specified StringBuilder.
     * <p>
     * Implementations are free to return a new StringBuilder if they can detect in
     * advance that the specified StringBuilder is too small.
     */
     ...
     * @param patternSelector       Allows different patterns to be used based on
     ...
     * @param charset               The character set. The platform default is used
     *                              if not specified.
     ...
     *                              {@link System#console()} is null, do not output
     *                              ANSI escape codes
     * @param headerPattern         The footer to place at the top of the document,
     *                              once.
     * @param footerPattern         The footer to place at the bottom of the
     *                              document, once.
    ...
    /**
     * Creates a LayoutTest using the default options. These options include using
     * UTF-8, the default conversion pattern, exceptions being written, and with
     * ANSI escape codes.
     *
     * @return the LayoutTest.
    ...
    /**
     * Creates a LayoutTest using the default options and the given configuration.
     * These options include using UTF-8, the default conversion pattern, exceptions
     * being written, and with ANSI escape codes.
     *
    ...
    /**
     * Custom LayoutTest builder. Use the {@link CustomPatternLayout#newBuilder()
